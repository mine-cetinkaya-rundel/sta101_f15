...
download("http://stat.duke.edu/courses/Fall15/sta101.002/rmd/lab7.Rmd", destfile = "lab7.Rmd")
```
</div>
...
gen_inc_stats <- acs_emp %>%
  group_by(gender) %>%
  summarise(xbar = mean(income), s = sd(income), n = n())
```

In this code chunk we not only calculated these summary statistics, but we also saved them in
a new data frame called `gen_inc_stats`. Let's view this data frame:

```{r gender-income-summary-view}
gen_inc_stats
```
...
```{r}
inference(data = acs, x = gender, y = income, type = "ci",
...
```{r}
inference(data = acs, x = gender, y = income, type = "ht", null = 0,
...
```{r}
qplot(data = acs_emp, x = emp_type, geom = "bar")
```
...
```{r key}
qplot(data = acs_emp, x = emp_type, fill = gender, position = "fill")
```
...
```{r}
inference(data = acs_emp_ft, x = gender, y = income, type = "ht", null = 0,
          method = "theoretical", statistic = "mean", alternative = "twosided")
```
...
```{r}
inference(data = acs_emp_pt, x = gender, y = income, type = "ht", null = 0,
          method = "theoretical", statistic = "mean", alternative = "twosided")
```
...
```{r}
inference(data = acs_emp, x = race, y = income, type = "ht", null = 0,
          method = "theoretical", statistic = "mean", alternative = "greater")
```
    

-   Conduct this hypothesis test using the `inference` function. Note that the response
    variable is `income` and the explanatory variable is `race`. You will need to figure out 
    the remaining arguments for the function. Use a trial-and-error approach, and let the 
    errors inform you as to what else needs to be specified, and how. (Note: Use the dataset
    containing records from all employes participants: `acs_emp`)
...
-   Pick **another** numerical variable from the dataset to be your response variable, and 
    ...
    research question, and interpret your conclusion in context of the dataset. Note that you c
    can use the complete `acs` dataset, the subsetted `acs_emp` dataset, or another subset that     you create.
</div>