ci_single_median_sim <- function(y, conf_level = 0.95, 
  ...
  if(boot_method == "perc"){
    ...
    # return
    return(list(median = round(median, 4), CI = round(ci, 4)))
  }
  ...
  if(boot_method == "se"){
    ...
    # return
    return(list(median = round(median, 4), SE = round(se, 4), ME = round(me, 4), CI = round(ci, 4)))
  }  
}