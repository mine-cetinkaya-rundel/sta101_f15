...
already loaded in your console's enviromment from last time, but note that in order to create a 
...
**[Insert Shiny app with sliders for $p$ and $n$ to demonstrate, remove
all following code, but keep some of the narrative and the question.]**
...
1.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.\
    *Hint:* Remember that R has functions such as `mean` to calculate summary
    statistics.

1.  Repeat the above simulation three more times but with modified sample
...
-   Answer the following two questions using the `inference` function. As 
    ...
    **a.** Is there convincing evidence that Spain has seen a change in its 
    atheism index between 2005 and 2012? If you find a significant difference,
    also quantify this difference with a condfidence interval.\
    ...
    **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012? If you find a significant 
    difference, also quantify this difference with a condfidence interval.
