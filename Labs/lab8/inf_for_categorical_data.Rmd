...
output:
  html_document:
    css: ~/Desktop/Teaching/Sta101_F15/Labs/lab.css
    highlight: pygments
    theme: cerulean
---
...
download("http://stat.duke.edu/courses/Fall15/sta101.002/rmd/lab8.Rmd", destfile = "lab8.Rmd")
```
</div>
...
**Ex 1.**  In the first paragraph, several key findings are reported. Do these 
    percentages appear to be *sample statistics* (derived from the data 
    sample) or *population parameters*? Explain your reasoning.

**Ex 2.**  The title of the report is "Global Index of Religiosity and Atheism". To
...
**Ex 3.**  What does each row of Table 6 correspond to? What does each row of 
...
**Ex 4.**  Using the command below, create a new dataframe called `us12` that contains
...
**Ex 5.**  Write out the conditions for inference to construct a 95% confidence
...
**Ex 6.**  Based on the R output, what is the margin of error for the estimate of the 
    proportion of the proportion of atheists in US in 2012?

**Ex 7.**  Using the `inference` function, calculate confidence intervals for the 
...
The first step is to make a varianle `p` that is a sequence from 0 to 1 with 
...
**Ex 8.**  Describe the relationship between `p` and `me`. Include the margin of
...
**Ex 9.**  Describe the sampling distribution of sample proportions at $n = 300$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.

**Ex 10.**  Keep $n$ constant and change $p$. How does the shape, center, and spread 
...
**Ex 11.**  Now also change $n$.How does $n$ appear to affect the distribution of $\hat{p}$?

**Ex 12.**  If you refer to Table 6, you'll find that Australia has a sample 
...
-   Is there convincing evidence that Spain has seen a change in its atheism index 
    ...
    *Hint:* First a new data set for respondents from Spain.

-   Is there convincing evidence that the US has seen a change in its atheism index 
    between 2005 and 2012? As always, write out the hypotheses for any tests you 
    conduct and outline the status of the conditions for inference. If you find a 
    significant difference, also quantify this difference with a condfidence interval. \
    *Hint:* First a new data set for respondents from Spain.
