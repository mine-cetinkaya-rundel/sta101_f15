...
\item The width of the confidence interval for $\hat{y}$ increases as
\begin{itemize}
...
\begin{itemize}
\item Degrees of freedom for the slope(s) in regression is $df = n - p - 1$ where $p $ is the number of 
predictors (explanatory variables) in the model.
\end{itemize}
